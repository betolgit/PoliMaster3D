...
// // 定时器设置
// const int TEMP_UPDATE_INTERVAL = 5000; // 5秒
...
//   // 启动定时器
//   temperatureTicker.attach_ms(TEMP_UPDATE_INTERVAL, updateTemperature);
...
//     html += "setInterval(function() { fetch('/getTemperature').then(response => response.text()).then(data => document.getElementById('temperature').innerText = data); }, " + String(TEMP_UPDATE_INTERVAL) + ");";
//     html += "</script>";
//     html += "</body></html>";
